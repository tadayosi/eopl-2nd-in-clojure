...
typedIds
  ...
  ;
typedId
  : optionalArgTexp=optionalTypeExp id=IDENTIFIER -> ^(TYPED_ID $optionalArgTexp $id)
  ;
procedureDeclaration
  : optionalResultTexp=optionalTypeExp procName=IDENTIFIER '(' ids=typedIds ')' '=' body=expression
    -> ^(PROC_DECL $optionalResultTexp $procName $ids $body)
  ;
primitive
  : '+' | '-' | '*' | 'add1' | 'sub1' | 'zero?'
  ;
