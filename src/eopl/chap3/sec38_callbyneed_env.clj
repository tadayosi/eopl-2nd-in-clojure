(ns eopl.chap3.sec38-callbyneed-env
  (:use eopl.chap3.sec38-callbyneed-grammar)
  (:import (eopl.chap3.sec38_callbyneed_grammar Closure ARef DirectTarget)))
...
(defn extend-env-recursively [proc-names idss bodies old-env]
  (let [len (count proc-names)
    ...
    (do
      (doseq [pos (range len)]
        (swap! vec conj (DirectTarget. (Closure.
                                         (nth idss pos)
                                         (nth bodies pos)
                                         env))))
      env)))
